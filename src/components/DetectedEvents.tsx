...
export default function DetectedEvents({ deviceId, apiKey }: DetectedEventsProps) {
  ...
  if (loading) {
    return <div className="p-4 text-center text-gray-600">Loading detection data...</div>
  }

  if (!detections.length) {
    return <div className="p-4 text-center text-gray-500">No detections recorded yet.</div>
  }
    ...
    <div className="bg-white rounded-md shadow p-4 space-y-6 border border-green-200">
      ...
      {modalImage && (
        <div className="fixed inset-0 bg-black bg-opacity-75 z-50 flex items-center justify-center">
          <div className="relative max-w-4xl w-full p-4">
            <button
              className="absolute top-4 right-4 text-white text-xl bg-black bg-opacity-40 hover:bg-opacity-60 p-2 rounded"
      ...
      <div>
        ...
        <div className="space-y-2">
          <div className="text-sm text-gray-600">{new Date(latest.timestamp).toLocaleString()}</div>
          ...
          <img
            ...
            alt="Snapshot"
            className="w-full max-h-60 object-cover rounded-md cursor-pointer"
          ...
          {detections.slice(1).map((event, index) => (
            ...
            >
              <img
                ...
                onClick={() => setModalImage(`${backendHost}/${event.snapshotPath}`)}
              />
